...
**背景**：当FTP服务器配置为主备集群时（通过/etc/hosts实现同一主机名映射多个IP），连接池需支持主备自动切换。
...
| 类 | 方法 | 功能 |
|----|------|------|
| `FtpPool` | `resolveHostWithFailover()` | 通过`InetAddress.getAllByName()`解析主机名获取所有IP |
| `FtpPool` | `createClientWithFailover()` | 按顺序尝试每个IP，失败则尝试下一个 |
| `FtpPool` | `isClientValid()` | 通过`sendNoOp()`验证连接有效性 |
| `FtpPool` | `checkAllClients()` | 定时健康检查，移除失效连接 |
...
getClient()
  ↓
遍历 availableClients → sendNoOp()验证
  ↓
无效则移除，有效则返回
  ↓
无可用连接 → createClientWithFailover()
  ↓
getAllByName("ftp.example.com")
  ↓
...
| 时机 | 保护机制 | 代码位置 |
|------|---------|----------|
| 获取连接时 | validate + sendNoOp() | `FtpPool.getClient()` |
| 归还连接时 | 有效性检查 | `FtpPool.returnClient()` |
| 健康检查时 | 定时检测 | `FtpPool.checkAllClients()` |
| 操作失败时 | 标记dead移除 | `FtpClient.handleConnectionFailure()` |
